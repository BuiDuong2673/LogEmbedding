...
class Sent2Vec:
    ...
    def __init__(
        self, W1: np.array, W2: np.array, word_ngrams: int = 2, dropout_k: int = 2, bucket_size: int = 2000000
    ):
    ...
    def get_sentence_embedding(self, word_indices: list, apply_dropout: bool = True) -> np.array:
        ...
        if apply_dropout and len(features) > self.dropout_k:
            ...
            indices_to_drop = np.random.choice(
                len(features), size=min(self.dropout_k, len(features) - 1), replace=False
            )
    ...
    def train_with_negative_sampling(
        self, sentence_word_indices: list, context_sentence_indices: list, negative_samples: list, learning_rate=0.01
    ):
